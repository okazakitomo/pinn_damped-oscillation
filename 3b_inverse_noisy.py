...
# Model update
fw = open('result/loss_3b_inverse_noisy.txt', 'w')
...
# Save
torch.save(net, 'result/net_3b_inverse_noisy.ckpt')
loss, loss_ode, loss_ic, loss_data = loss_function(net, t_coll, t_data, u_data, m, c_est, k, u0, v0)
...
plt.legend()
plt.title('Inverse: Noisy Data')
plt.savefig('figure/3b_inverse_noisy.png')
plt.close()

# Learning curve
loss = np.loadtxt('result/loss_3b_inverse_noisy.txt').T
plt.figure()
...
plt.title('Inverse: Noisy Data')
plt.legend()
plt.savefig('figure/loss_3b_inverse_noisy.png')
plt.close()
...
plt.title('Inverse: Noisy Data')
plt.legend()
plt.savefig('figure/3b_inverse_noisy_c.png')
plt.close()
